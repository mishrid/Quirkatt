...
class Move {
    ...
    static final int MAX_INDEX = SIDE * SIDE;

    /** Constants used to compute linearized indices. */
    private static final int STEP_C = 1, STEP_R = 5,
    ...
    /** Return a Move FROM index k TO index K. */
    static Move move(int from, int to) {
        return move(col(from), row(from), col(to), row(to));
    }

    /** Return a Move FROM index k TO index K with a NEXTJUMP. */
    static Move move(int from, int to, Move nextJump) {
        return move(col(from), row(from), col(to), row(to), nextJump);
    }
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            if (move1.col0() == move0.col0() && move1.row0() == move0.row0()) {
                return move1;
            } else {
                return move0;
            }
    ...
    /** Return the capturing move (a jump). */
    Move hasNextJump() {
        return _nextJump;
    }
    ...
    boolean isLeft() {
        ...
        return _col1 == _col0 - 1 && _row1 == _row0
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    }
    ...
    boolean isForwardMove() {
        return _row1 == _row0 + 1 && _col0 == _col1
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    }

    /** Return true iff this is a backward, non-capturing move
     *  to the right. */
    boolean isBackwardMove() {
        return _row1 == _row0 - 1 && _col0 == _col1
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    }

    /** Return true iff this is a north-west, non-capturing move
     *  to the right. */
    boolean isNorthWest() {
        return _col1 == _col0 + 1 && _row1 == _row0 + 1
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    }

    /** Return true iff this is a north-east, non-capturing move
     *  to the right. */
    boolean isNorthEast() {
        return _col1 == _col0 - 1 && _row1 == _row0 + 1
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    ...
    /** Return true iff this is a south-east, non-capturing move
     *  to the right. */
    boolean isSouthEast() {
        return _col1 == _col0 + 1 && _row1 == _row0 - 1
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    ...
    /** Return true iff this is a south-west, non-capturing move
     *  to the right. */
    boolean isSouthWest() {
        return _col1 == _col0 - 1 && _row1 == _row0 - 1
                && validSquare(_col1, _row1) && validSquare(_col0, _row0);
    ...
    /** Return true iff this is a valid jump from an even index. */
    boolean isEvenJump() {
        if (!(validSquare(_col1, _row1) && validSquare(_col0, _row0))) {
            return false;
        }

        if (_col1 == _col0 + 2 && _row1 == _row0) {
            return true;
        }

        if (_col1 == _col0 && _row1 == _row0 - 2) {
            return true;
        }
    ...
    /** Return true iff this is a valid jump from an odd index. */
    boolean isOddJump() {
        if (!(validSquare(_col1, _row1) && validSquare(_col0, _row0))) {
            return false;
        }
        ...
        if (_col1 == _col0 + 2 && _row1 == _row0 + 2) {
            return true;
        }
    ...
    private void toString(Formatter out) {
        String result = "";
        result = result + _col0 + _row0 + "-" + _col1 + _row1;
        boolean flag = !(_nextJump == null);
        Move move = this;
        while (flag) {
            move = move._nextJump;
            result = result + "-" + move.col1() + move.row1();
            flag = !(move._nextJump == null);
        }
        out.format("%s", result);
    }
    ...
    /** Returns the linearized index of the final jump. */
    public int finalJumpIndex() {
        int k;
        Move  move = this;
        while (move.hasNextJump() != null) {
            move = move.hasNextJump();
        }
        k = move.toIndex();
        return k;
    }

    /** Returns the linearized index of the final jumped index. */
    public int finalJumpedIndex() {
        int k;
        Move  move = this;
        while (move.hasNextJump() != null) {
            move = move.hasNextJump();
