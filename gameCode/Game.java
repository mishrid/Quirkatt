...
class Game {
    ...
    void process() {
        ...
        while (true) {
            ...
            while (_state != SETUP && !_board.gameOver()) {
                ...
                if (_state == PLAYING) {
                    if (move != null) {
                        PieceColor change = _board.whoseMove();
                        _board.makeMove(move, true);
                        if (change == _board.whoseMove()) {
                            _reporter.errMsg("that was an illegal move.");
                        }
                    }
                }
            }
    ...
    void doMove(String[] operands) {
        PieceColor change = _board.whoseMove();
        _board.makeMove(Move.parseMove(operands[0]), true);
    }
    ...
    void doSet(String[] operands) {
        PieceColor player;
        if (operands[0].toLowerCase().equalsIgnoreCase("white")) {
            player = WHITE;
        } else {
            player = BLACK;
        }
